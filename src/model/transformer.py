...
class TransformerDecoder(nn.Module):
    ...
    def generate(self, encoded, lang_id, max_len=200, sample=False, temperature=None, return_score=False):
        """
        ...
        sent_scores = torch.FloatTensor(bs).fill_(0)
        if is_cuda:
            decoded = decoded.cuda()
            unfinished_sents = unfinished_sents.cuda()
            lengths = lengths.cuda()
            sent_scores = sent_scores.cuda()
        ...
        while cur_len < max_len:
            ...
            if return_score:
                cur_scores = F.log_softmax(scores, dim=-1).data
                cur_scores = (torch.gather(cur_scores, 1, next_words.long())).squeeze(1)
                sent_scores = sent_scores + cur_scores
                assert sent_scores.size() == (bs,)

            next_words = next_words.squeeze(1)
        ...
        return decoded[:cur_len], lengths, sent_scores
